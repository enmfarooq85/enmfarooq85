# 👋 Hi, I'm Muhammad Farooq

🎓 Computer Science Student | 💻 Full Stack Engineer (MERN, Next.js, React Native)

---

## 🚀 About Me
I am a passionate Computer Science student who loves building modern, scalable, and high performance applications. I enjoy working across the full stack and creating solutions that solve real world problems.

I have hands on experience in both **web and mobile development**, and I continuously improve my skills through real projects and problem solving.

---

## 💡 What I Do
- Build **full-stack web applications** using the MERN stack  
- Develop **cross platform mobile apps** using React Native  
- Design **clean and responsive user interfaces**  
- Create **secure and scalable backend systems**  
- Optimize performance and improve user experience  

---

## 🛠 Tech Stack

### 🌐 Frontend
React.js, Next.js, Tailwind CSS, ShadCN, MUI, Ant Design  

### ⚙️ Backend
Node.js, Express.js, Nest.js, GraphQl, REST APIs, Authentication, JWT 

### 🗄 Database
MongoDB, Mongoose, Firebase, Supabase, Appwrite 

### 📱 Mobile
React Native, Expo  

### 🔧 Tools
Git, GitHub, Postman, Figma, Vercel, Netlify, Render, Railway

---

## 🧠 Problem Solving & Competitive Programming
- Solved **320+ problems** on LeetCode  
- Active in **DSA & Competitive Programming (Codeforces)**  
- Strong focus on problem solving and logical thinking

---

## 🔭 Currently Learning
- Python ecosystem (**FastAPI, Django, Numpy, Pandas**)  
- Artificial Intelligence & Machine Learning (AI/ML)
- System Design & Backend Architecture
- Diving Deep into the Mobile App Developement

---

## 🎯 Goals
- Become a well rounded software engineer  
- Build impactful and scalable products

---

## 📫 Connect With Me
- GitHub: https://github.com/enmfarooq85
- Leetcode: https://leetcode.com/u/enmfarooq85
- Codeforces: https://www.codeforces.com/profile/muhammadfarooq85
- LinkedIn: https://linkedin.com/in/enmfarooq85
- Twitter: https://www.x.com/enmfarooq85
- Discord: https://www.discord.com/users/1361174333699133472
- Monkeytype: https://www.monkeytype.com/profile/enmfarooq85
- Youtube: https://youtube.com/@enmfarooq85
- Portfolio: https://enmfarooq85.vercel.app
- Email: enmuhammadfarooq@gmail.com

---

## 📊 GitHub Stats

<!-- GitHub Streak Stats -->
<p align="left">
<img width="100%" src="https://github-readme-streak-stats.herokuapp.com/?user=enmfarooq85&theme=tokyonight&hide_border=true" />
</p>

<!-- GitHub Graph -->
[![Muhammad Farooq's github activity graph](https://github-readme-activity-graph.vercel.app/graph?username=enmfarooq85&bg_color=0d1117&color=58a6ff&line=58a6ff&point=ffffff&area=true&hide_border=true)](https://github.com/ashutosh00710/github-readme-activity-graph)
